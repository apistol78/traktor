...
Ref< ISerializable > SceneEditorPageFactory::cloneAsset(const ISerializable* asset) const
{
	...
	{
		world::Traverser::visit(mutableSceneAsset, [&](Ref< world::EntityData >& inoutEntityData) -> world::Traverser::Result {
	...
	else if (auto mutableEntityData = dynamic_type_cast< world::EntityData* >(mutableAsset))
	{
		...
		// Rename all entities.
		world::Traverser::visit(mutableEntityData, [&](Ref< world::EntityData >& inoutEntityData) -> world::Traverser::Result {
			const Guid newEntityId = Guid::create();
			inoutEntityData->setId(newEntityId);
			return world::Traverser::Result::Continue;
		});
