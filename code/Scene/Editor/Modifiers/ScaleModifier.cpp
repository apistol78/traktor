...
)
{
	...
	Scalar cameraDistance = worldTransform.translation().length();
	Scalar guideLength = max(cameraDistance * c_guideScale, c_guideMinLength);

	int mouseX, mouseY, mouseZ;
	float scaleX, scaleY, scaleZ;
	getMouseDeltaAxises(
		worldTransform * primitiveRenderer->getView(),
...
{
	Matrix44 worldTransform = outTransform;
	...
	outTransform = scale(Vector4(1.0f, 1.0f, 1.0f, 0.0f) + editDelta) * outTransform;
}
