...
#include <dirent.h>
#include <errno.h>
#include "Core/Io/iPhone/NativeVolume.h"
#include "Core/Io/iPhone/NativeStream.h"
...
void NativeVolume::mountVolumes(FileSystem& fileSystem)
{
	...
	if (mainBundle)
	{
		...
		char buffer[4096];
		[bundlePath getCString: buffer maxLength: sizeof_array(buffer) encoding: NSUTF8StringEncoding];
	
		workingDirectory = std::wstring(L"C:") + mbstows(buffer);	
	}
