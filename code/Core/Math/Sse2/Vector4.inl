...
{
	const Vector4 tmp = shuffle< 0, 0, 0, 0 >();
	return Scalar(tmp.m_data);
}

T_MATH_INLINE Scalar Vector4::y() const
{
	const Vector4 tmp = shuffle< 1, 1, 1, 1 >();
	return Scalar(tmp.m_data);
}

T_MATH_INLINE Scalar Vector4::z() const
{
	const Vector4 tmp = shuffle< 2, 2, 2, 2 >();
	return Scalar(tmp.m_data);
}

T_MATH_INLINE Scalar Vector4::w() const
{
	const Vector4 tmp = shuffle< 3, 3, 3, 3 >();
	return Scalar(tmp.m_data);
}
...
T_MATH_INLINE Vector4 Vector4::absolute() const
{
	...
	const __m128 mask = _mm_load_ps((const float *)c_absoluteMask);
	return Vector4(_mm_and_ps(m_data, mask));
}
...
{
	T_ASSERT(out);
	const __m128i tmp = _mm_cvtps_epi32(m_data);
...
T_MATH_INLINE Vector4 Vector4::operator - () const
{
	...
	const __m128 mask = _mm_load_ps((const float *)c_negateMask);
...
{
	const __m128 eq =_mm_cmpeq_ps(m_data, v.m_data);
...
{
	const Vector4 tmp = v + v.shuffle< 1, 0, 3, 2 >();
...
{
	const __m128 mask = _mm_cmpge_ps(condition.m_data, _mm_setzero_ps());
