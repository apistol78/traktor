...
bool MeshAssetRasterizer::generate(const editor::IEditor* editor, const MeshAsset* asset, drawing::Image* outImage) const
{
	...
	const Matrix44 modelView = translate(0.0f, 0.0f, 1.75f) * scale(invMaxExtent, invMaxExtent, invMaxExtent) * rotateY(/*asset->getPreviewAngle()*/3.0f) * translate(-boundingBox.getCenter());
