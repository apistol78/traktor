...
public:
	...
	{
		std::wstring name;
		uint32_t channel = c_InvalidIndex;
		...
		explicit Map(const std::wstring& name_, uint32_t channel_, bool anisotropic_ = true, const Guid& texture_ = Guid(), drawing::Image* image_ = nullptr)
		:	name(name_)
		,	channel(channel_)
