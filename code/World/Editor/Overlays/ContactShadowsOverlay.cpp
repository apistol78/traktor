...
#include "World/Editor/Overlays/ContactShadowsOverlay.h"
...
void ContactShadowsOverlay::setup(render::RenderGraph& renderGraph, render::ScreenRenderer* screenRenderer, IWorldRenderer* worldRenderer, const WorldRenderView& worldRenderView, float alpha) const
{
	...
	rp->addBuild([=](const render::RenderGraph& renderGraph, render::RenderContext* renderContext) {
		...
		pp->setTextureParameter(c_handleDebugTexture, contactShadowsTargetSet->getColorTexture(0));
