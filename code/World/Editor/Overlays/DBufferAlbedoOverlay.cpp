...
#include "World/Editor/Overlays/DBufferAlbedoOverlay.h"
...
T_IMPLEMENT_RTTI_FACTORY_CLASS(L"traktor.world.DBufferAlbedoOverlay", 0, DBufferAlbedoOverlay, IDebugOverlay)

bool DBufferAlbedoOverlay::create(resource::IResourceManager* resourceManager)
{
...
void DBufferAlbedoOverlay::setup(render::RenderGraph& renderGraph, render::ScreenRenderer* screenRenderer, IWorldRenderer* worldRenderer, const WorldRenderView& worldRenderView, float alpha) const
{
	...
	rp->addBuild([=](const render::RenderGraph& renderGraph, render::RenderContext* renderContext) {
		...
		pp->setTextureParameter(c_handleDebugTexture, dbufferTargetSet->getColorTexture(0));
