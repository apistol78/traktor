...
void ShadowMaskOverlay::setup(render::RenderGraph& renderGraph, render::ScreenRenderer* screenRenderer, World* world, IWorldRenderer* worldRenderer, const WorldRenderView& worldRenderView, float alpha, float mip) const
{
	const render::RGTargetSet shadowMaskId = findTargetByName(renderGraph, L"Shadow mask");
	if (shadowMaskId == render::RGTargetSet::Invalid)
	{
