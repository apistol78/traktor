...
T_IMPLEMENT_RTTI_FACTORY_CLASS(L"traktor.world.ShadowMapOverlay", 0, ShadowMapOverlay, IDebugOverlay)

bool ShadowMapOverlay::create(resource::IResourceManager* resourceManager)
{
...
void ShadowMapOverlay::setup(render::RenderGraph& renderGraph, render::ScreenRenderer* screenRenderer, IWorldRenderer* worldRenderer, const WorldRenderView& worldRenderView, float alpha, float mip) const
{
	...
	rp->addBuild([=](const render::RenderGraph& renderGraph, render::RenderContext* renderContext) {
		...
		pp->setTextureParameter(c_handleDebugTexture, shadowMapTargetSet->getDepthTexture());
