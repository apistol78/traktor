...
ValueAllocator< TransformValue > s_valueAllocator;

	}

T_IMPLEMENT_RTTI_CLASS(L"traktor.jungle.TransformValue", TransformValue, IValue)

void* TransformValue::operator new (size_t size)
{
	T_ASSERT(size == sizeof(TransformValue));
	return s_valueAllocator.alloc();
}

void TransformValue::operator delete (void* ptr)
{
