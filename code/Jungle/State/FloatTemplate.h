...
public:
	explicit FloatTemplate(const std::wstring& tag, float threshold = std::numeric_limits< float >::max());

	explicit FloatTemplate(const std::wstring& tag, float threshold, float min, float max, FloatTemplatePrecision precision, bool cyclic);
