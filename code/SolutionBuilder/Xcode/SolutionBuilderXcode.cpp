...
)
{
	...
	for (RefArray< ProjectItem >::const_iterator i = items.begin(); i != items.end(); ++i)
	{
		...
		else if (const sb::File* file = dynamic_type_cast< const sb::File* >(*i))
		{
			std::set< Path > files;
...
void SolutionBuilderXcode::generatePBXCopyFilesBuildPhaseSection(OutputStream& s, const Solution* solution, const RefArray< Project >& projects) const
{
	const RefArray< Aggregation >& aggregations = solution->getAggregations();
...
void SolutionBuilderXcode::generatePBXResourcesBuildPhaseSection(OutputStream& s, const RefArray< Project >& projects) const
{
...
void SolutionBuilderXcode::generatePBXShellScriptBuildPhaseSection(OutputStream& s, const Solution* solution, const RefArray< Project >& projects) const
{
	Path projectPath = FileSystem::getInstance().getAbsolutePath(solution->getRootPath() + m_rootSuffix);
...
void SolutionBuilderXcode::generateXCBuildConfigurationSection(OutputStream& s, const Solution* solution, const RefArray< Project >& projects) const
{
	...
	for (RefArray< Project >::const_iterator i = projects.begin(); i != projects.end(); ++i)
	{
		...
		if (!plistFile.empty())
			log::info << L"Using plist file \"" << plistFile << L"\"" << Endl;
...
bool SolutionBuilderXcode::includeFile(OutputStream& s, const Path& fileName, int32_t indent) const
{
