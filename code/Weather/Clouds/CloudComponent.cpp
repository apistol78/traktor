...
struct ImpostorUpdateRenderBlock : public render::RenderBlock
{
	...
	virtual void render(render::IRenderView* renderView) const
	{
		...
		renderView->beginPass(impostorTargetSet, 0, &clear, render::TfColor, render::TfColor);
...
)
{
	...
	for (uint32_t i = 0; i < impostorSliceCount; ++i)
	{
		...
		m_impostorTargets[i] = renderSystem->createRenderTargetSet(targetDesc, nullptr, T_FILE_LINE_W);
...
)
{
	...
	)
	{
		...
		for (uint32_t slice = 0; slice < lastUpdateSlice; ++slice)
		{
			...
			ImpostorUpdateRenderBlock* impostorRenderBlock = renderContext->allocNamed< ImpostorUpdateRenderBlock >(L"Cloud impostor update");
			...
			for (uint32_t i = 0; i < sliceParticleCount; )
			{
				...
				for (uint32_t pass = 0; pass < sizeof_array(c_techniquePasses); ++pass)
				{
					...
					auto particleRenderBlock = renderContext->allocNamed< render::IndexedRenderBlock >(L"Cloud particles");
					...
					worldRenderPass.setProgramParameters(particleRenderBlock->programParams, transform, transform);
			...
			renderContext->draw(impostorRenderBlock);
		}
	...
	for (uint32_t slice = 0; slice < m_impostorSliceCount; ++slice)
	{
		...
		auto renderBlock = renderContext->allocNamed< render::NonIndexedRenderBlock >(L"Cloud impostors");
		...
		worldRenderPass.setProgramParameters(renderBlock->programParams, transform, transform);
