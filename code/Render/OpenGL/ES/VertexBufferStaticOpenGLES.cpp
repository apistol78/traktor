...
struct DeleteVertexArrayCallback : public ContextOpenGLES::IDeleteCallback
{
	GLuint m_arrayName;
...
T_IMPLEMENT_RTTI_CLASS(L"traktor.render.VertexBufferStaticOpenGLES", VertexBufferStaticOpenGLES, VertexBufferOpenGLES)

VertexBufferStaticOpenGLES::VertexBufferStaticOpenGLES(ContextOpenGLES* context, const AlignedVector< VertexElement >& vertexElements, uint32_t bufferSize)
...
VertexBufferStaticOpenGLES::~VertexBufferStaticOpenGLES()
{
	destroy();
}

void VertexBufferStaticOpenGLES::destroy()
{
	if (m_bufferObject)
	{
		if (m_context)
...
void* VertexBufferStaticOpenGLES::lock()
{
	m_lockOffset = 0;
...
void VertexBufferStaticOpenGLES::activate(StateCache* stateCache)
{
	if (!m_bufferObject)
	{
