...
Ref< ContextOpenGLES > ContextOpenGLES::createContext(const SystemApplication& sysapp, const RenderViewDefaultDesc& desc)
{
	...
	if (!context->m_window->create(desc.displayMode.width, desc.displayMode.height))
		return nullptr;
	...
	context->m_window->show();

	context->m_display = eglGetDisplay(context->m_window->getDisplay());
	...
	context->m_surface = eglCreateWindowSurface(context->m_display, context->m_config, context->m_window->getWindow(), 0);
...
bool ContextOpenGLES::reset(int32_t width, int32_t height)
{
	...
	if (m_window)
	{
		...
		m_surface = eglCreateWindowSurface(m_display, m_config, (EGLNativeWindowType)m_window->getWindow(), 0);
	}
