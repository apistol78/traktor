...
struct DeleteProgramCallback : public ContextOpenGLES::IDeleteCallback
{
...
T_IMPLEMENT_RTTI_CLASS(L"traktor.render.ProgramOpenGLES", ProgramOpenGLES, IProgram)

ProgramOpenGLES* ProgramOpenGLES::ms_current = 0;

ProgramOpenGLES::~ProgramOpenGLES()
{
	destroy();
}

Ref< ProgramOpenGLES > ProgramOpenGLES::create(ContextOpenGLES* resourceContext, const ProgramResource* resource)
{
...
void ProgramOpenGLES::setFloatParameter(handle_t handle, float param)
{
	setFloatArrayParameter(handle, &param, 1);
}

void ProgramOpenGLES::setFloatArrayParameter(handle_t handle, const float* param, int length)
{
...
void ProgramOpenGLES::setVectorParameter(handle_t handle, const Vector4& param)
{
	setVectorArrayParameter(handle, &param, 1);
}

void ProgramOpenGLES::setVectorArrayParameter(handle_t handle, const Vector4* param, int length)
{
	SmallMap< handle_t, uint32_t >::iterator i = m_parameterMap.find(handle);
	if (i == m_parameterMap.end())
		return;
...
void ProgramOpenGLES::setMatrixParameter(handle_t handle, const Matrix44& param)
{
	setMatrixArrayParameter(handle, &param, 1);
}

void ProgramOpenGLES::setMatrixArrayParameter(handle_t handle, const Matrix44* param, int length)
{
	SmallMap< handle_t, uint32_t >::iterator i = m_parameterMap.find(handle);
	if (i == m_parameterMap.end())
		return;
...
void ProgramOpenGLES::setTextureParameter(handle_t handle, ITexture* texture)
{
	SmallMap< handle_t, uint32_t >::iterator i = m_parameterMap.find(handle);
	if (i != m_parameterMap.end())
...
void ProgramOpenGLES::setStencilReference(uint32_t stencilReference)
{
...
bool ProgramOpenGLES::activate(StateCache* stateCache, float targetSize[2], float postTransform[4], bool invertCull)
{
	...
	for (uint32_t i = 0; i < nsamplers; ++i)
	{
		...
		else if (CubeTextureOpenGLES* ct = dynamic_type_cast< CubeTextureOpenGLES* >(resolved))
			binding = static_cast< ITextureBinding* >(ct);
		else if (VolumeTextureOpenGLES* vt = dynamic_type_cast< VolumeTextureOpenGLES* >(resolved))
			binding = static_cast< ITextureBinding* >(vt);
		else if (RenderTargetOpenGLES* rt = dynamic_type_cast< RenderTargetOpenGLES* >(resolved))
			binding = static_cast< ITextureBinding* >(rt);
		else if (RenderTargetDepthOpenGLES* rtd = dynamic_type_cast< RenderTargetDepthOpenGLES* >(resolved))
...
ProgramOpenGLES::ProgramOpenGLES(ContextOpenGLES* resourceContext, GLuint program, const ProgramResource* resource)
