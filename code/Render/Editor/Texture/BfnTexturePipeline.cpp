...
{
	const float q = std::floor(value * 127.5f + 127.5f);
...
Vector4 findMinimumQuantizationError(const Vector4& unit)
{
	...
	{
		const float t = float(i) / (128 - 1);

		const Vector4 probe = unit * Scalar(t);
		const Vector4 quantized(
			quantize255(probe.x()),
			quantize255(probe.y()),
			quantize255(probe.z())
		);
		const Vector4 diff = (quantized - probe).absolute() / Scalar(t);

		const float error = max< float >(diff.x(), diff.y(), diff.z());
		if (error < minError)
		{
			out = quantized;
			minError = error;
...
) const
{
	...
	{
		const int32_t size = asset->m_size;
		...
		if (!asset->m_collapseSymmetry)
		{
			...
			{
				const float nv = float(v) / (size - 1) * 2.0f - 1.0f;
				for (int32_t u = 0; u < size; ++u)
				{
					const float nu = float(u) / (size - 1) * 2.0f - 1.0f;
					const Vector4 n = Vector4(nu, 1.0f, nv).normalized();
					const Vector4 qn = findMinimumQuantizationError(n);
					const float qnln = qn.length();
