...
// Produced by scripts/generate-node-emitter-capture.py from
// code/Render/Editor/Glsl/GlslEmitter.cpp and the shader node headers.
...
{
	static const std::map< std::wstring, std::wstring > s_capture = {
		{ L"Abs", LR"NODESRC(bool emitAbs(GlslContext& cx, Abs* node)
{
	...
	return true;
		...
		{ L"Add", LR"NODESRC(bool emitAdd(GlslContext& cx, Add* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ArcusCos", LR"NODESRC(bool emitArcusCos(GlslContext& cx, ArcusCos* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ArcusTan", LR"NODESRC(bool emitArcusTan(GlslContext& cx, ArcusTan* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ArrayElement", LR"NODESRC(bool emitArrayElement(GlslContext& cx, ArrayElement* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Clamp", LR"NODESRC(bool emitClamp(GlslContext& cx, Clamp* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Color", LR"NODESRC(bool emitColor(GlslContext& cx, Color* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ComputeOutput", LR"NODESRC(bool emitComputeOutput(GlslContext& cx, ComputeOutput* node)
{
	cx.enterCompute();
	...
	return true;
		...
		{ L"Conditional", LR"NODESRC(bool emitConditional(GlslContext& cx, Conditional* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Cos", LR"NODESRC(bool emitCos(GlslContext& cx, Cos* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Cross", LR"NODESRC(bool emitCross(GlslContext& cx, Cross* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Derivative", LR"NODESRC(bool emitDerivative(GlslContext& cx, Derivative* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Discard", LR"NODESRC(bool emitDiscard(GlslContext& cx, Discard* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"DispatchIndex", LR"NODESRC(bool emitDispatchIndex(GlslContext& cx, DispatchIndex* node)
{
	if (!cx.inCompute())
	{
	...
	return true;
		...
		{ L"Div", LR"NODESRC(bool emitDiv(GlslContext& cx, Div* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Dot", LR"NODESRC(bool emitDot(GlslContext& cx, Dot* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Exp", LR"NODESRC(bool emitExp(GlslContext& cx, Exp* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Fraction", LR"NODESRC(bool emitFraction(GlslContext& cx, Fraction* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"FragmentPosition", LR"NODESRC(bool emitFragmentPosition(GlslContext& cx, FragmentPosition* node)
{
	if (!cx.inFragment())
	{
	...
	return true;
		...
		{ L"FrontFace", LR"NODESRC(bool emitFrontFace(GlslContext& cx, FrontFace* node)
{
	if (!cx.inFragment())
	{
	...
	return true;
		...
		{ L"IndexedUniform", LR"NODESRC(bool emitIndexedUniform(GlslContext& cx, IndexedUniform* node)
{
	...
	return true;
		...
		{ L"Instance", LR"NODESRC(bool emitInstance(GlslContext& cx, Instance* node)
{
	if (cx.inVertex())
	{
		auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	else
	{
		...
		{ L"Interpolator", LR"NODESRC(bool emitInterpolator(GlslContext& cx, Interpolator* node)
{
	if (!cx.inFragment())
	{
	...
	return true;
		...
		{ L"Iterate", LR"NODESRC(bool emitIterate(GlslContext& cx, Iterate* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Iterate2", LR"NODESRC(bool emitIterate2(GlslContext& cx, Iterate2* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Iterate2d", LR"NODESRC(bool emitIterate2d(GlslContext& cx, Iterate2d* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	std::wstring inputName;
	...
	*out = GlslVariable(out->getNode(), out->getName(), input->getType());
	...
	return true;
		...
		{ L"Length", LR"NODESRC(bool emitLength(GlslContext& cx, Length* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Lerp", LR"NODESRC(bool emitLerp(GlslContext& cx, Lerp* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Log", LR"NODESRC(bool emitLog(GlslContext& cx, Log* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MatrixIn", LR"NODESRC(bool emitMatrixIn(GlslContext& cx, MatrixIn* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MatrixOut", LR"NODESRC(bool emitMatrixOut(GlslContext& cx, MatrixOut* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Max", LR"NODESRC(bool emitMax(GlslContext& cx, Max* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MemberValue", LR"NODESRC(bool emitMemberValue(GlslContext& cx, MemberValue* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Min", LR"NODESRC(bool emitMin(GlslContext& cx, Min* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MixIn", LR"NODESRC(bool emitMixIn(GlslContext& cx, MixIn* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MixOut", LR"NODESRC(bool emitMixOut(GlslContext& cx, MixOut* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Mul", LR"NODESRC(bool emitMul(GlslContext& cx, Mul* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"MulAdd", LR"NODESRC(bool emitMulAdd(GlslContext& cx, MulAdd* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Neg", LR"NODESRC(bool emitNeg(GlslContext& cx, Neg* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Normalize", LR"NODESRC(bool emitNormalize(GlslContext& cx, Normalize* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Parameter", LR"NODESRC(bool emitParameter(GlslContext& cx, Parameter* node)
{
	...
	return true;
		...
		{ L"PixelOutput", LR"NODESRC(bool emitPixelOutput(GlslContext& cx, PixelOutput* node)
{
	...
	return true;
		...
		{ L"Polynomial", LR"NODESRC(bool emitPolynomial(GlslContext& cx, Polynomial* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Pow", LR"NODESRC(bool emitPow(GlslContext& cx, Pow* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ReadStruct", LR"NODESRC(bool emitReadStruct(GlslContext& cx, ReadStruct* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"ReadStruct2", LR"NODESRC(bool emitReadStruct2(GlslContext& cx, ReadStruct2* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"RecipSqrt", LR"NODESRC(bool emitRecipSqrt(GlslContext& cx, RecipSqrt* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Reflect", LR"NODESRC(bool emitReflect(GlslContext& cx, Reflect* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Repeat", LR"NODESRC(bool emitRepeat(GlslContext& cx, Repeat* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	std::wstring inputName;
	...
	return true;
		...
		{ L"Round", LR"NODESRC(bool emitRound(GlslContext& cx, Round* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
		...
		{ L"Scalar", LR"NODESRC(bool emitScalar(GlslContext& cx, Scalar* node)
{
	...
	return true;
		...
		{ L"Script", LR"NODESRC(bool emitScript(GlslContext& cx, Script* node)
{
	...
	return true;
		...
		{ L"Sign", LR"NODESRC(bool emitSign(GlslContext& cx, Sign* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Sin", LR"NODESRC(bool emitSin(GlslContext& cx, Sin* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Sqrt", LR"NODESRC(bool emitSqrt(GlslContext& cx, Sqrt* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Step", LR"NODESRC(bool emitStep(GlslContext& cx, Step* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Struct", LR"NODESRC(bool emitStruct(GlslContext& cx, Struct* node)
{
	Ref< GlslVariable > out = cx.getShader().createStructVariable(
		node->findOutputPin(L"Output"),
		node->getParameterName(),
	...
	return true;
		...
		{ L"Sub", LR"NODESRC(bool emitSub(GlslContext& cx, Sub* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Sum", LR"NODESRC(bool emitSum(GlslContext& cx, Sum* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	std::wstring inputName;
	...
	return true;
		...
		{ L"Switch", LR"NODESRC(bool emitSwitch(GlslContext& cx, Switch* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Swizzle", LR"NODESRC(bool emitSwizzle(GlslContext& cx, Swizzle* node)
{
	...
	return true;
		...
		{ L"Tan", LR"NODESRC(bool emitTan(GlslContext& cx, Tan* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"TargetSize", LR"NODESRC(bool emitTargetSize(GlslContext& cx, TargetSize* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	Ref< GlslVariable > out = cx.emitOutput(node, L"Output", GlslType::Float2);
	...
	return true;
		...
		{ L"TextureSize", LR"NODESRC(bool emitTextureSize(GlslContext& cx, TextureSize* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Transform", LR"NODESRC(bool emitTransform(GlslContext& cx, Transform* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Transpose", LR"NODESRC(bool emitTranspose(GlslContext& cx, Transpose* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Truncate", LR"NODESRC(bool emitTruncate(GlslContext& cx, Truncate* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	...
	return true;
		...
		{ L"Uniform", LR"NODESRC(bool emitUniform(GlslContext& cx, Uniform* node)
{
	Ref< GlslVariable > out = cx.getShader().createVariable(
		node->findOutputPin(L"Output"),
		node->getParameterName(),
	...
	return true;
		...
		{ L"Vector", LR"NODESRC(bool emitVector(GlslContext& cx, Vector* node)
{
	auto& f = cx.getShader().getOutputStream(GlslShader::BtBody);
	comment(f, node);
	...
	return true;
		...
		{ L"VertexInput", LR"NODESRC(bool emitVertexInput(GlslContext& cx, VertexInput* node)
{
	if (!cx.inVertex())
	{
	...
	return true;
		...
		{ L"VertexOutput", LR"NODESRC(bool emitVertexOutput(GlslContext& cx, VertexOutput* node)
{
	cx.enterVertex();
...
{
	static const std::map< std::wstring, std::wstring > s_summary = {
		{ L"Abs", L"Output = abs(Input)" },
		{ L"Add", L"Output = Input1 + Input2" },
		{ L"ArcusCos", L"Output = acos(Theta)" },
		{ L"ArcusTan", L"Output = atan(XY.x, XY.y)" },
		{ L"ArrayElement", L"Output = Input[Index]" },
		{ L"Cos", L"Output = cos(Theta)" },
		{ L"Cross", L"Output = cross(Input1, Input2)" },
		{ L"Discard", L"Output = Pass" },
		{ L"Div", L"Output = Input1 / Input2" },
		{ L"Dot", L"Output = dot(Input1, Input2)" },
		{ L"Exp", L"Output = exp(Input)" },
		{ L"Fraction", L"Output = fract(Input)" },
		{ L"FragmentPosition", L"Output = gl_FragCoord.xy" },
		{ L"FrontFace", L"Output = gl_FrontFacing ? 1.0 : 0.0" },
		{ L"Interpolator", L"Output = Input" },
		{ L"Length", L"Output = length(Input)" },
		{ L"Lerp", L"Output = mix(Input1, Input2, Blend)" },
		{ L"Max", L"Output = max(Input1, Input2)" },
		{ L"Min", L"Output = min(Input1, Input2)" },
		{ L"Mul", L"Output = Input1 * Input2" },
		{ L"MulAdd", L"Output = fma(Input1, Input2, Input3)" },
		{ L"Neg", L"Output = -Input" },
		{ L"Normalize", L"Output = normalize(Input)" },
		{ L"Pow", L"Output = pow(max(Input, 0.0), Exponent)" },
		{ L"RecipSqrt", L"Output = inversesqrt(Input)" },
		{ L"Reflect", L"Output = reflect(Direction, Normal)" },
		{ L"Round", L"Output = round(Input)" },
		{ L"Sign", L"Output = sign(Input)" },
		{ L"Sin", L"Output = sin(Theta)" },
		{ L"Sqrt", L"Output = sqrt(Input)" },
		{ L"Step", L"Output = step(Edge, X)" },
		{ L"Sub", L"Output = Input1 - Input2" },
		{ L"Tan", L"Output = tan(Theta)" },
		{ L"TargetSize", L"Output = _vk_targetSize.xy" },
		{ L"Transform", L"Output = Transform * Input" },
		{ L"Transpose", L"Output = transpose(Input)" },
	};
	return s_summary;
}
...
{
	static const std::set< std::wstring > s_deprecated = {
		L"IndexedUniform",
		L"ReadStruct",
		L"ReadStruct2",
		L"Struct",
