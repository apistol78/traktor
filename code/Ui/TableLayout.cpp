	...
	dimensionVector_t& h)
{
	...
	for (int32_t r = 0; r < nr; ++r)
	{
		if (rdef.empty() || rdef[r % rdef.size()] == 0)
		{
	...
	if (rdef.empty())
	{
		// No row definition; make all rows same height.
		const int32_t mxh = *std::max_element(h.begin(), h.end());
		for (auto& it : h)
			it = mxh;
	}
...
bool TableLayout::fit(Widget* widget, const Size& bounds, Size& result)
{
	...
	const int32_t nc = (int32_t)m_cdef.size();
	...
	{
		const int32_t c = i % std::max(nc, 1);
		const int32_t r = i / std::max(nc, 1);

		const Size pf = children[i]->getPreferredSize(bounds);
		const Size sz(
...
void TableLayout::update(Widget* widget)
{
	...
	const Size avail = inner.getSize() - margin - margin - Size(pad.cx * (nc - 1), pad.cy * (nr - 1));
	...
	for (int32_t i = 0; i < (int32_t)children.size(); ++i)
	{
		...
		const Size pf = children[i]->getPreferredSize(avail);
		const Size mx = children[i]->getMaximumSize();
