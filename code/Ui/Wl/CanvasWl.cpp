...
void CanvasWl::setForeground(const Color4ub& foreground)
{
	m_foreground = foreground;
}

void CanvasWl::setBackground(const Color4ub& background)
{
	m_background = background;
}

void CanvasWl::setFont(const Font& font)
{
	if (font != m_font)
	{
		m_font = font;
...
const IFontMetric* CanvasWl::getFontMetric() const
{
	return this;
}

void CanvasWl::setLineStyle(LineStyle lineStyle)
{
}

void CanvasWl::setPenThickness(int thickness)
{
...
void CanvasWl::resetClipRect()
{
	cairo_reset_clip(m_cr);
}

void CanvasWl::drawPixel(int x, int y, const Color4ub& c)
{
	setSourceColor(c);
...
void CanvasWl::drawLine(int x1, int y1, int x2, int y2)
{
...
void CanvasWl::drawLines(const Point* pnts, int npnts)
{
	setSourceColor(m_foreground);
...
void CanvasWl::drawCurve(const Point& start, const Point& control, const Point& end)
{
	const Bezier2nd b2(
...
void CanvasWl::fillCircle(int x, int y, float radius)
{
...
void CanvasWl::drawEllipticArc(int x, int y, int w, int h, float start, float end)
{
}

void CanvasWl::drawSpline(const Point* pnts, int npnts)
{
	setSourceColor(m_foreground);
...
void CanvasWl::fillRect(const Rect& rc)
{
	setSourceColor(m_background);
	cairo_rectangle(m_cr, rc.left, rc.top, rc.getWidth(), rc.getHeight());
	cairo_fill(m_cr);
...
void CanvasWl::fillGradientRect(const Rect& rc, bool vertical)
{
	setSourceColor(m_background);
	cairo_rectangle(m_cr, rc.left, rc.top, rc.getWidth(), rc.getHeight());
	cairo_fill(m_cr);
...
void CanvasWl::drawRect(const Rect& rc)
{
	setSourceColor(m_foreground);
	cairo_rectangle(m_cr, rc.left, rc.top, rc.getWidth(), rc.getHeight());
	cairo_stroke(m_cr);
...
void CanvasWl::fillPolygon(const Point* pnts, int count)
{
	setSourceColor(m_background);
	cairo_move_to(m_cr, pnts[0].x, pnts[0].y);
	for (int i = 1; i < count; ++i)
...
void CanvasWl::drawBitmap(const Point& dstAt, const Point& srcAt, const Size& size, ISystemBitmap* bitmap, BlendMode blendMode, Filter filter, uint8_t alpha)
{
	BitmapWl* bm = static_cast< BitmapWl* >(bitmap);
	T_ASSERT(bm);
...
void CanvasWl::drawGlyph(const Point& at, const wchar_t chr)
{
	drawText(at, std::wstring(1, chr));
}

void* CanvasWl::getSystemHandle()
{
	T_FATAL_ERROR;
	return nullptr;
}

void CanvasWl::getAscentAndDescent(int32_t& outAscent, int32_t& outDescent) const
{
	if (realizeFont())
	{
		cairo_font_extents_t x;
...
int32_t CanvasWl::getAdvance(wchar_t ch, wchar_t next) const
{
	if (realizeFont())
	{
...
int32_t CanvasWl::getLineSpacing() const
{
	if (realizeFont())
	{
		cairo_font_extents_t x;
...
Size CanvasWl::getExtent(const std::wstring& text) const
{
	if (realizeFont())
	{
...
void CanvasWl::setSourceColor(const Color4ub& color)
{
...
bool CanvasWl::realizeFont() const
{
	if (!m_fontDirty)
		return true;
