...
set MAKE_OS=windows
set MAKE_INCLUDE=$(TRAKTOR_HOME)\resources\build\configurations\make-config-pnacl-nmake.inc
...
%SOLUTIONBUILDER% ^
	...
	%TRAKTOR_HOME%\resources\build\ExternPNaCl.xms

%SOLUTIONBUILDER% ^
	-f=make2 ^
	-make-solution-template=%TRAKTOR_HOME%\resources\build\configurations\make-solution.sbm ^
	-make-project-template=%TRAKTOR_HOME%\resources\build\configurations\make-project-emscripten.sbm ^
	%TRAKTOR_HOME%\resources\build\TraktorPNaCl.xms
