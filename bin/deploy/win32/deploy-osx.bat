...
if "%1"=="build" (
	...
	%TRAKTOR_HOME%\bin\win32\RemoteDeploy %DEPLOY_TARGET_HOST% launch-osx.sh > %DEPLOY_PROJECT_ROOT%\deploy.log
	popd
	...
	%TRAKTOR_HOME%\bin\win32\RemoteLaunch %DEPLOY_TARGET_HOST% launch-osx.sh %DEPLOY_EXECUTABLE% >> %DEPLOY_PROJECT_ROOT%\deploy.log
