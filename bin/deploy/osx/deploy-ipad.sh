...
if [[ ${1} == "build" ]]; then
	...
	$TRAKTOR_HOME/bin/osx/RemoteDeploy $DEPLOY_TARGET_HOST launch-ipad.sh >> $DEPLOY_PROJECT_ROOT/deploy.log
	popd
	...
	$TRAKTOR_HOME/bin/osx/RemoteLaunch $DEPLOY_TARGET_HOST launch-ipad.sh "$DEPLOY_EXECUTABLE.app" >> $DEPLOY_PROJECT_ROOT/deploy.log
