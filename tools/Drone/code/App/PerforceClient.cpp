...
public:
	...
	{
		log::info << L"Perforce info \"" << mbstows(data) << L"\"" << Endl;
	}

	virtual void OutputError(const char* errBuf)
	{
		log::error << L"Perforce operation failed \"" << mbstows(errBuf) << L"\"" << Endl;
		m_failed = true;
		m_outLastError = mbstows(errBuf);
	}
	...
	virtual void OutputStat(StrDict* varList)
	{
		for (int i = 0; ; ++i)
		{
			...
			log::info << L"\"" << mbstows(var.Text()) << L"\" => \"" << mbstows(val.Text()) << L"\"" << Endl;
		}
	}
#endif
...
T_IMPLEMENT_RTTI_CLASS(L"traktor.drone.PerforceClient", PerforceClient, Object)
...
PerforceChangeList* PerforceClient::createChangeList(const std::wstring& description)
{
	if (!establishConnection())
		return 0;
...
bool PerforceClient::establishConnection()
{
	...
	m_p4client.SetProg("Traktor.Drone");
	...
	log::info << L"Init P4 client..." << Endl;
	...
	if (e.Test())
	{
		...
		log::error << L"Unable to connect to Perforce; \"" << mbstows(buffer.Text()) << L"\"" << Endl;
	...
	{
		log::info << L"Login P4 client..." << Endl;
	...
	log::info << L"P4 connected successfully" << Endl;
